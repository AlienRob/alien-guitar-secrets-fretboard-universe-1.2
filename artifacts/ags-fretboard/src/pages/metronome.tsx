...
// ── Beat Block ────────────────────────────────────────────────────────────────
function BeatBlock({
  beatState, segs, activeSeg = -1, onTap, accent, accentDim, stripDim,
}: {
  ...
  return (
    <div onClick={onTap} style={{
      ...
      <div style={{ height: 11, backgroundColor: stripCol, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ flex: 1, display: "flex" }}>
        {Array.from({ length: segs }).map((_, j) => {
          ...
          const track   = isFirst && !isMute ? (isAccent ? `${accentDim}60` : `${accentDim}30`) : "transparent";
          return (
            <div key={j} style={{
              flex: 1,
      ...
      {(isAccent || isMute) && (
        <div style={{ position: "absolute", bottom: 3, right: 3, fontSize: 7, fontWeight: 700, letterSpacing: 0.5, color: isMute ? "#444" : accent, opacity: 0.85 }}>
          {isMute ? "GAP" : "ACC"}
        </div>
      )}
    </div>
...
function CountRow({ segs, states, numBeats, accent, accentDim }: {
  segs: number; states: BeatState[]; numBeats: number; accent: string; accentDim: string;
}) {
        ...
        <div key={bi} style={{ flex: 1, display: "flex" }}>
          {subs.map((label, si) => (
            <div key={si} style={{
              ...
              color: si === 0 ? (states[bi] === "accent" ? accent : accentDim) : "#40406a",
...
export default function MetronomePage() {
  ...
  const handlePlayStop = () => {
    ...
    ensureAudioCtx();
    if (playing) {
      eng.stop(); setPlaying(false); setCurrentBeat(null);
    } else {
      eng.bpm             = bpm;
      eng.beatsPerMeasure = numBeats;
      eng.subdivision     = segs;
  ...
  const handleTimeSig = (idx: number) => {
    ...
    if (!isPremium && !FREE_TIME_SIGS.some((f) => f.label === ts.label)) {
      setUpgradeBanner("Extended time signatures"); return;
    }
    setTimeSigIdx(idx); setShowBeats(false); restartEngine();
  };

  const handleSubdiv = (idx: number) => {
    if (!isPremium && idx !== 0) { setUpgradeBanner("Subdivisions"); return; }
    setSubdivIdx(idx); setShowSubdiv(false); restartEngine();
  };

  const handleSound = (idx: number) => { setSoundIdx(idx); setShowSound(false); };

  const handleBeatTap = (bi: number) => {
    if (!isPremium && bi !== 0) { setUpgradeBanner("Per-beat accent & mute"); return; }
    setBeatStates((prev) => {
      const next = [...prev];
  ...
  const getAngle = useCallback((cx: number, cy: number, ex: number, ey: number) =>
    Math.atan2(ex - cx, cy - ey) * (180 / Math.PI), []);

  const onJogPointerDown = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ensureAudioCtx();
    const rect = e.currentTarget.getBoundingClientRect();
    const cx = rect.left + rect.width / 2, cy = rect.top + rect.height / 2;
    dragRef.current = { startAngle: getAngle(cx, cy, e.clientX, e.clientY), startBpm: bpm };
  ...
  const onJogPointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const rect = e.currentTarget.getBoundingClientRect();
    const cx = rect.left + rect.width / 2, cy = rect.top + rect.height / 2;
    let delta = getAngle(cx, cy, e.clientX, e.clientY) - dragRef.current.startAngle;
  ...
  const glowColor = !playing || activeBeatState === "mute" ? "#222"
    : activeBeatState === "accent" ? theme.accent : `${theme.accent}66`;

  const BG = `linear-gradient(135deg, ${BG_MAIN} 0%, ${BG_GRAD} 50%, ${BG_MAIN} 100%)`;

  return (
    <div className="min-h-screen flex flex-col" style={{ background: BG, position: "relative", overflow: "hidden", margin: "-16px -16px -16px -16px" }}>
      ...
      {/* Header */}
      <div className="flex items-center justify-center px-4 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", position: "relative", zIndex: 1 }}>
        <div className="text-center">
          <p className="text-xs font-semibold tracking-widest" style={{ color: theme.accent }}>ALIEN GUITAR SECRETS</p>
      ...
      {/* Upgrade banner */}
      {upgradeBanner && (
        <div style={{ position: "relative", zIndex: 1, margin: "8px 16px 0", padding: "10px 16px", borderRadius: 10, background: "rgba(106,0,255,0.15)", border: "1px solid rgba(106,0,255,0.4)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          <span style={{ fontSize: 13, color: "#c084fc" }}>{upgradeBanner} — premium feature</span>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <Link href="/pricing" style={{ fontSize: 12, fontWeight: 700, color: "#fff", background: "#6a00ff", padding: "4px 12px", borderRadius: 6, textDecoration: "none", letterSpacing: 1 }}>
              UPGRADE
            </Link>
            <button onClick={() => setUpgradeBanner(null)} style={{ background: "none", border: "none", color: "#6a5a9a", cursor: "pointer", fontSize: 16 }}>×</button>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col items-center gap-3 px-4 py-3 pb-8" style={{ position: "relative", zIndex: 1, maxWidth: 480, margin: "0 auto", width: "100%" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0" }}>
          ...
          {THEMES.map((t, i) => (
            <button key={i} onClick={() => {
              if (!isPremium && i !== 4) { setUpgradeBanner("Colour themes"); return; }
              setThemeIdx(i as ThemeIdx);
            }}
              title={t.label}
              style={{
                width: i === themeIdx ? 22 : 16, height: i === themeIdx ? 22 : 16,
        ...
        <div style={{ width: "100%" }}>
          <div style={{ display: "flex", gap: 6 }}>
            {beatStates.map((state, bi) => (
              <BeatBlock key={bi}
                ...
                accent={theme.accent} accentDim={theme.accentDim} stripDim={theme.stripDim}
              />
            ))}
            {/* Global A button */}
            <div onClick={() => handleBeatTap(0)} style={{
          ...
          <CountRow segs={segs} states={beatStates} numBeats={numBeats} accent={theme.accent} accentDim={theme.accentDim} />
        </div>
        ...
        <div className="flex gap-2 w-full relative">
          ...
          <div className="flex-1 relative">
            <button onClick={() => { setShowBeats((v) => !v); setShowSubdiv(false); setShowSound(false); }}
              style={{
          ...
          {/* SUBDIV */}
          <div className="flex-1 relative">
            <button onClick={() => { setShowSubdiv((v) => !v); setShowBeats(false); setShowSound(false); }}
              style={{
                width: "100%", height: 48, borderRadius: 10,
          ...
          {/* SOUND */}
          <div className="flex-1 relative">
            <button onClick={() => { setShowSound((v) => !v); setShowBeats(false); setShowSubdiv(false); }}
              style={{
                width: "100%", height: 48, borderRadius: 10,
        ...
        {/* Jog wheel */}
        <div ref={jogRef} style={{ width: 160, height: 160, touchAction: "none", cursor: "grab", flexShrink: 0 }}
          onPointerDown={onJogPointerDown} onPointerMove={onJogPointerMove}
          onPointerUp={onJogPointerUp} onPointerCancel={onJogPointerUp}>
        ...
        <div className="flex gap-2 justify-center">
          {[-10,-5,-1,+1,+5,+10].map((d) => (
            <button key={d} onClick={() => {
              ...
              background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.45)", border: "none",
            }}>
              {d > 0 ? `+${d}` : d}
            </button>
          ))}
