...
export default function TunerPage() {
  ...
  const handleInstPick = (inst: Instrument) => {
    if (!isPremium && inst !== "guitar") { setUpgradeFeature("Bass & Ukulele support"); return; }
    setInstrument(inst);
  };

  const handleStrings = (sc: string) => {
    if (!isPremium && sc !== "6") { setUpgradeFeature("7, 8 and 12-string tuning"); return; }
    setStringCount(sc);
  };

  const handleA4 = (v: number) => {
    if (!isPremium && v !== 440) { setUpgradeFeature("Custom A4 reference pitch"); return; }
    setA4(v);
  };
    ...
    <div className="tuner-root">
      ...
      <div className="tuner-phone" ref={phoneRef}>
        <main className="tuner-screen">
          ...
          <header className="tuner-header" style={{ justifyContent: "space-between" }}>
            <button
              onClick={() => { if (running) stop(); navigate("/"); }}
              style={{ background: "none", border: "none", cursor: "pointer", padding: "4px 8px", opacity: 0.55, flexShrink: 0 }}
              aria-label="Back"
            >
                ...
                <div className="tuner-sheet-list">
                  {(tunings[instrument][stringCount] ?? []).map((t, i) => (
                    <button
                      ...
                      onClick={() => {
                        if (!isPremium && i > 0) { setUpgradeFeature("Alternate tunings"); return; }
